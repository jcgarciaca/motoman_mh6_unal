...
void printAxisAngle(Eigen::Vector3d axis, double angle_rot);
...
//	matrix.getRPY(roll, pitch, yaw, 1);
	...
	double angle_rot = q.getAngle();

	Eigen::Vector3d axis(q.getAxis());

	printAxisAngle(axis, angle_rot);

	//double axis[3] = q.getAxis();
...
void printAxisAngle(Eigen::Vector3d axis, double angle_rot){
	...
	std::cout << "angle: " << angle_rot << std::endl;
	std::cout << "--------------------------------------" << std::endl;
}
